...
```bash
npm install lint-cli -g
```

安装完成后，可执行 `lint-cli -h` 以验证安装成功。

### 使用

#### `lint-cli init`：一键接入

在项目根目录执行 `lint-cli init`，即可一键接入规范，为项目安装规范 `Lint` 所需的依赖和配置。
...
> 注 1：如果项目已经配置过 ESLint、stylelint 等 Linter，执行 `lint-cli init` 将会提示存在冲突的依赖和配置，并在得到确认后进行覆盖：
>
> 注 2：如果项目的 .vscode/ 目录被 .gitignore 忽略，可以在拉取项目后单独执行 `lint-cli init --vscode` 命令写入 `.vscode/extensions.json` 和 `.vscode/settings.json` 配置文件
...
> 注 1：事实上，你可以在任意目录执行 `lint-cli scan` `lint-cli` 会根据文件类型、JSON 等特征嗅探项目类型。但我们还是推荐在执行过 `lint-cli init` 的项目根目录执行 `lint-cli scan`，以得到最准确的扫描结果。
>
> 注 2: `lint-cli` 会根据项目内有无 eslint 和 stylelint 配置文件判断使用项目的配置文件还是 `lint-cli` 默认配置进行扫描。若使用项目的，在未安装依赖时会帮其安装（执行 npm i）。若使用项目配置扫描失败，则使用默认配置扫描
...
#### `lint-cli commit-file-scan` 提交文件扫描
...
#### `lint-cli commit-msg-scan` 提交信息扫描
...
- lint-cli.init(config)：将项目一键接入规范，效果等同于 `lint-cli init`
...
`lint-cli` 基于一份配置进行扫描（但你也可以零配置使用），支持的配置参数有：
...
`lint-cli` 会读取执行目录下的 `lint-cli.config.js` 作为配置文件。`lint-cli init` 会在执行目录下新增如下的 `lint-cli.config.js` 文件：
...
如果你的 TS 项目 commit 卡口和 `lint-cli scan` 运行时间很长，可以通过如下在 `.eslintrc.js` 中增加以下配置提升性能：
